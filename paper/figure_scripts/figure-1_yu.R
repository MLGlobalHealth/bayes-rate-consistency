...
# COVIMOD
covimod_survey <- load_covimod_data()
covimod_data <- readRDS("data/COVIMOD/COVIMOD-single.rds")
...
# COVID cases
df.covid.cases <- readr::read_csv("data/time-series-covid19-confirmed-germany.csv")
dt.covid.cases <- as.data.table(df.covid.cases)
dt.covid.cases <- dt.covid.cases[date <= "2020-08-01"]

# COVID deaths
df.covid.deaths <- readr::read_csv("data/time-series-covid19-deaths-germany.csv")
dt.covid.deaths <- as.data.table(df.covid.deaths)
dt.covid.deaths <- dt.covid.deaths[date <= "2020-08-01"]

# Stringency index
df.covid.idx <- readr::read_csv("data/owid-covid-data.csv")
...
p_cases <- ggplot(dt.covid.cases, aes(x = date)) +
  ...
  geom_line(aes(y=seven_day_avg, color="Daily cases"),  size = 0.7) +
...
##### Sample size #####
covimod_data <- readRDS("data/COVIMOD/COVIMOD-multi.rds")
dt.part <- covimod_data$offsets
...
p_sample <- ggplot(dt.rep, aes(wave, N)) +
  ...
  scale_fill_manual(values = c("steel blue", "#2A9D8F", "#E9C46A", "#F4A261", "#E76F51")) +
...
patchwork <- patchwork + plot_annotation(tag_levels = "A") & theme(plot.tag = element_text(size = 10))

ggsave( "paper/figures/figure-1.png", patchwork,

...
transformed parameters
{
  ...
  for (t in 1:T){
    f[t, MM] = restruct_hsgp(A, gp_sigma[t,MM], gp_rho_1[t,MM], gp_rho_2[t,MM],
                             L1, L2, M1, M2, PHI1, PHI2,
                             z[t, 1:M2,], NN_IDX);
    f[t, FF] = restruct_hsgp(A, gp_sigma[t,FF], gp_rho_1[t,FF], gp_rho_2[t,FF],
                             L1, L2, M1, M2, PHI1, PHI2,
                             z[t, (M2+1):2*M2,], NN_IDX);
    f[t, MF] = restruct_hsgp(A, gp_sigma[t,MF], gp_rho_1[t,MF], gp_rho_2[t,MF],
                             L1, L2, M1, M2, PHI1, PHI2,
                             z[t, (2*M2+1):3*M2,], NN_IDX);
    ...
    // Add 65 year old offsets
    log_cnt_rate[t, MM][:,65] = log_cnt_rate[t, MM][:,65] + zeta[MM];
    log_cnt_rate[t, FF][:,65] = log_cnt_rate[t, FF][:,65] + zeta[FF];
    log_cnt_rate[t, MF][:,65] = log_cnt_rate[t, MF][:,65] + zeta[FF];
    log_cnt_rate[t, FM][:,65] = log_cnt_rate[t, FM][:,65] + zeta[MM];

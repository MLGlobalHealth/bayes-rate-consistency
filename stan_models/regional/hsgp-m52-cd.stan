...
data
{
  int<lower=1> N_U, N_R; // Number of observations (U: Urban, R: Rural)
...
{
  vector<lower=0>[R] gp_alpha = tan(gp_alpha_unif); // Reparametrize Half-Cauchy for optimization in HMC

  matrix[A,A] f_U, f_R;
  array[R] matrix[A,A] log_cnt_rate;

  { // Local scope
    f_U = hsgp_matern52(A, gp_alpha[U], gp_rho_1[U], gp_rho_2[U],
                        L1, L2, M1, M2, PHI1, PHI2, z[1:M1,]);
    f_R = hsgp_matern52(A, gp_alpha[R], gp_rho_1[R], gp_rho_2[R],
...
model
{
  ...
  { // Local scope
    ...
    vector[N] alpha_strata_flat = append_row(

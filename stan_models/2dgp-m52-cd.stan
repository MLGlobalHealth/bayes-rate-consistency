...
{
  vector<lower=0>[G-1] gp_alpha = tan(gp_alpha_unif); // Reparametrize Half-Cauchy

  matrix[A,A] f_MM, f_FF, f_MF;
  array[G] matrix[A,A] log_cnt_rate;
  array[G] matrix<lower=0>[A,C] alpha_strata;

  f_MM = gp2d_matern52(age_idx_std, age_idx_std, gp_alpha[MM], gp_rho_1[MM], gp_rho_2[MM], z[1:A,]);
  f_FF = gp2d_matern52(age_idx_std, age_idx_std, gp_alpha[FF], gp_rho_1[FF], gp_rho_2[FF], z[(A+1):2*A,]);
  f_MF = gp2d_matern52(age_idx_std, age_idx_std, gp_alpha[MF], gp_rho_1[MF], gp_rho_2[MF], z[(2*A+1):3*A,]);

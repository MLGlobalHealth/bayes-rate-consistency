...
transformed data {
  ...
  matrix[A, M2] PHI3 = PHI(A, M2, L3, age_idx_std);
...
transformed parameters {
  ...
  for (t in 1:T){
    f[t, MM] = hsgp_matern32_restruct(A, gp_sigma[t,MM], gp_rho_1[t,MM], gp_rho_2[t,MM],
                             L1, L2, M1, M2, PHI1, PHI2, z[t, 1:M2,], NN_IDX);
    f[t, FF] = hsgp_matern32_restruct(A, gp_sigma[t,FF], gp_rho_1[t,FF], gp_rho_2[t,FF],
                             L1, L2, M1, M2, PHI1, PHI2, z[t, (M2+1):2*M2,], NN_IDX);
    f[t, MF] = hsgp_matern32_restruct(A, gp_sigma[t,MF], gp_rho_1[t,MF], gp_rho_2[t,MF],
    ...
    { // Local scope
      ...
      for(r in 1:t){ // Add repeat effects
        ...
        if(r > 1){
          f_[,r-1] = PHI3 * (diagSPD_Matern32(g_sigma[r-1], gp_rho[r-1], L3, M2) .* zz[r-1,]);
          rho[,r-1] = -exp(f_[,r-1] + rho_0[r-1]);
          mu_flat_MM[sm:em] = m_flat_MM[ROW_MAJOR_IDX_M[sm:em]] .* exp( rho[ROW_MAJOR_IDX_M_AGE[sm:em],r-1] + log_O_M[sm:em]);
          mu_flat_FF[sf:ef] = m_flat_FF[ROW_MAJOR_IDX_F[sf:ef]] .* exp( rho[ROW_MAJOR_IDX_F_AGE[sf:ef],r-1] + log_O_F[sf:ef]);
          mu_flat_MF[sm:em] = m_flat_MF[ROW_MAJOR_IDX_M[sm:em]] .* exp( rho[ROW_MAJOR_IDX_M_AGE[sm:em],r-1] + log_O_M[sm:em]);
          mu_flat_FM[sf:ef] = m_flat_FM[ROW_MAJOR_IDX_F[sf:ef]] .* exp( rho[ROW_MAJOR_IDX_F_AGE[sf:ef],r-1] + log_O_F[sf:ef]);
        } else {

...
parameters {
  ...
  matrix[R-1, A] rho; // repeated response effect
  ...
  for (t in 1:T){
    ...
    { // Local scope
      ...
      for(r in 1:t){ // Add repeat effects
        ...
        if(r > 1){  
          mu_flat_MM[sm:em] = m_flat_MM[ROW_MAJOR_IDX_M[sm:em]] .* to_vector(exp(rho[r-1, ROW_MAJOR_IDX_M_AGE[sm:em]])) .* exp(log_O_M[sm:em]); 
          mu_flat_FF[sf:ef] = m_flat_FF[ROW_MAJOR_IDX_F[sf:ef]] .* to_vector(exp(rho[r-1, ROW_MAJOR_IDX_F_AGE[sf:ef]])) .* exp(log_O_F[sf:ef]);
          mu_flat_MF[sm:em] = m_flat_MF[ROW_MAJOR_IDX_M[sm:em]] .* to_vector(exp(rho[r-1, ROW_MAJOR_IDX_M_AGE[sm:em]])) .* exp(log_O_M[sm:em]);
          mu_flat_FM[sf:ef] = m_flat_FM[ROW_MAJOR_IDX_F[sf:ef]] .* to_vector(exp(rho[r-1, ROW_MAJOR_IDX_F_AGE[sf:ef]])) .* exp(log_O_F[sf:ef]);
        } else {
...
model {
  ...
  for(a in 1:A){ target += normal_lpdf(rho[,a] | 0, 1) ;}  // Repeated response

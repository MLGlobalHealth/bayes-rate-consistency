...
parameters {
  ...
  matrix<lower = 0, upper = 1>[R-2, A] coef;
  vector<lower = 0, upper = 1>[A] rho_0;
  real sg;
...
transformed parameters {
  ...
  matrix<lower = 0, upper = 1>[R-1, A] rho_mat; // repeated response effect
  ...
  rho_mat[1, ] = to_row_vector(rho_0);
  for (i in 2:(R-1)){
    rho_mat[i,] = coef[i-1, ] .* rho_mat[i-1,] + sg;
  }
  
  rho = log(to_vector(rho_mat));
  ...
  for (t in 1:T){
    ...
    { // Local scope
      ...
      for(r in 1:t){ // Add repeat effects
        ...
        if(r > 1){
          mu_flat_MM[sm:em] = m_flat_MM[ROW_MAJOR_IDX_M[sm:em]] .* to_vector(rho_mat[r-1, ROW_MAJOR_IDX_M_AGE[sm:em]]) .* exp(log_O_M[sm:em]); 
          mu_flat_FF[sf:ef] = m_flat_FF[ROW_MAJOR_IDX_F[sf:ef]] .* to_vector(rho_mat[r-1, ROW_MAJOR_IDX_F_AGE[sf:ef]]) .* exp(log_O_F[sf:ef]);
          mu_flat_MF[sm:em] = m_flat_MF[ROW_MAJOR_IDX_M[sm:em]] .* to_vector(rho_mat[r-1, ROW_MAJOR_IDX_M_AGE[sm:em]]) .* exp(log_O_M[sm:em]);
          mu_flat_FM[sf:ef] = m_flat_FM[ROW_MAJOR_IDX_F[sf:ef]] .* to_vector(rho_mat[r-1, ROW_MAJOR_IDX_F_AGE[sf:ef]]) .* exp(log_O_F[sf:ef]);
        } else {
...
model {
  ...
  //for(r in 2:R-1){ target += normal_lpdf( to_vector(rho_mat[r, ]) | to_vector(coef[r-1,]).*to_vector(rho_mat[r-1,]), sg[r]);   }// Repeated response
  target += normal_lpdf(to_vector(coef) | 0, 1); 
  target += normal_lpdf(rho_0 | 0, 1); 
  target += normal_lpdf(sg| 0, 1);

...
parameters {
  ...
  matrix<upper=0>[R-1, A] rho; // repeated response effect
  real rho_0;
  real<lower=0> nu; // over disperison
  vector<lower=0>[R-1] sg; // over disperison
  ...
  for (t in 1:T){
    ...
    { // Local scope
      ...
      for(r in 1:t){ // Add repeat effects
        ...
        if(r > 1){  
          mu_flat_MM[sm:em] = m_flat_MM[ROW_MAJOR_IDX_M[sm:em]] .* exp(to_vector(rho[r-1, ROW_MAJOR_IDX_M_AGE[sm:em]]) + log_O_M[sm:em]); 
          mu_flat_FF[sf:ef] = m_flat_FF[ROW_MAJOR_IDX_F[sf:ef]] .* exp(to_vector(rho[r-1, ROW_MAJOR_IDX_F_AGE[sf:ef]]) + log_O_F[sf:ef]);
          mu_flat_MF[sm:em] = m_flat_MF[ROW_MAJOR_IDX_M[sm:em]] .* exp(to_vector(rho[r-1, ROW_MAJOR_IDX_M_AGE[sm:em]]) + log_O_M[sm:em]);
          mu_flat_FM[sf:ef] = m_flat_FM[ROW_MAJOR_IDX_F[sf:ef]] .* exp(to_vector(rho[r-1, ROW_MAJOR_IDX_F_AGE[sf:ef]]) + log_O_F[sf:ef]);
        } else {
...
model {
  ...
  //target += normal_lpdf(-rho | rho_0, sg);  // Repeated response
  for(r in 1:(R-1)){target += normal_lpdf(-rho[r,] | rho_0, sg[r]);  }
  target += cauchy_lpdf(sg| 0, 1);  // Repeated response

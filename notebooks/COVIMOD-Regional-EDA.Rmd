...
ggsave("../figures/map-of-germany.png", units = "cm", height = FIG_HEIGHT_CM, width = FIG_WIDTH_CM) # Save fig.
```
...
dt.agg[, PROP := COUNT / sum(COUNT), by = .(wave, URBN_TYPE)]
```
...
tmp <- dt.part[, .(N = .N), by=.(wave, rep)]
tmp[, pct := N/sum(N) * 100, by=wave]
tmp$rep <- ordered(tmp$rep, labels = c('First time', '1', '2', '3', '4 or more'))

ggplot(tmp, aes(wave, pct, fill = rep)) + 
  geom_bar(stat = "identity", position = "fill") + 
  scale_x_continuous(expand = c(0,0)) + 
  scale_y_continuous(expand = c(0,0), labels = scales::percent_format()) + 
  labs(x = 'Wave', y = 'Percentage of sample', fill = 'No. of repeat')
```

```{r}
tmp2 <- dt.agg[, .(COUNT = sum(COUNT)), by=.(wave, TYPE)] # Proportion of household contacts and non-household contact by urban type
tmp2[, PROP := COUNT / sum(COUNT), by = wave]
tmp2 <- tmp2[TYPE == "Household"]

tmp <- merge(tmp, tmp2, by = c("wave"))

tmp[, logit := log(PROP/(1-PROP))]

ggplot(tmp, aes(pct, logit)) +
  geom_point(aes(color = rep), alpha = 0.4)
```
```{r}
lm()
```
